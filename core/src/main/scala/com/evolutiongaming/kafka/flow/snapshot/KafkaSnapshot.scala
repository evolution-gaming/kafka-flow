...
/** Snapshot of the current state.
  ...
  *
  * I.e. we want to store snapshot metadata on every step of our folds, but we do not want to serialize it to
  * `ByteVector` each time unless we actually decided to save snapshot into persistence.
  */
