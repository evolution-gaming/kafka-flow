...
object FoldToState {
  ...
  def apply[F[_]: Monad: KeyContext, S, E](
    ...
    val foldAndPersist = fold.persistBatch[NonEmptyList](persistence).value
    val removeIfEmpty = foldAndPersist tap { (state, _) =>
      if (state.isEmpty) KeyContext[F].remove else ().pure[F]
    }
    val foldToState = removeIfEmpty.stateful(storage)
    foldToState(records).void
