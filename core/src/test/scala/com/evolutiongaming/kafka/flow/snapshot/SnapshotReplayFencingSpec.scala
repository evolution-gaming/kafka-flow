...
/** Deterministic (TestControl-driven, mocked clock) flow-level regressions for the compare-and-set replay-window fix.
  *
...
class SnapshotReplayFencingSpec extends FunSuite {
  ...
  /** Recovers `key` at offset 5 while the partition is (re)assigned at offset 0 (a slower key held the commit back),
    * then feeds `records` and fires the timers once, all under a mocked clock. Returns the stored snapshot afterwards.
    * `executeEmbed` raises if the flow fails (e.g. a stale-write conflict from the offset-gated store) - the bug.
    */
  private def runRecoveredAheadOfCommit(
    ...
    val program = for {
      ...
      _ <- PartitionFlow
        ...
        .use { flow =>
          // advance the (mocked) clock so the registered timer expires, then a poll fires it
