...
class KeySpec(val globalRead: GlobalRead) extends CassandraSpec {
  ...
  test("queries") { cassandra =>
    ...
    for {
      keys <-
        CassandraKeys.withSchema(cassandra.session, cassandra.sync, ConsistencyOverrides.none, CassandraKeys.DefaultSegments)
  ...
  test("failures") { cassandra =>
    for {
      ...
      keys      <-
        CassandraKeys.withSchema(session, cassandra.sync, ConsistencyOverrides.none, CassandraKeys.DefaultSegments)
      _ <- failAfter.set(1)
