...
| Cassandra persist-only (merged) | in master | P-1..P-3 yes (P-3 is a documented residual, not code) |
...
- **A-test.** An IT MUST hold a transaction genuinely open across the read and **assert the
  ...
  above the pin. *On the combined implementation branch, not here.*
...
- **B-test.** A takeover-abort IT MUST crash an owner mid-transaction under the partition's own
  ...
  **immediately after the successor's init** (`read_committed` = `read_uncommitted` end offsets —
  ...
  committed snapshot and excludes the dangling record. *On the combined implementation branch, not here.*
...
## P — Cassandra persist-only mode (merged upstream)
...
- **R-c (process): land A first, then stack the deadline; don't parallel-merge.** The deadline and A drafts both
  edit `readPartition` and add `ReadSnapshotsSpec`; the deadline and B drafts both edit `KafkaPersistenceModule`.
