...
### 1.1 The two Cassandra drafts: persist-only and full (neither merged)

Neither Cassandra arm is merged upstream — both are open drafts. The conservative one is
**persist-only** (`tj/…-cassandra-persist-only`), intended as the first step to merge. It gates every
`persist` (`IF offset <= :offset`, the first-write compound, and the F-1 guard-expired repair) but
leaves `delete` a plain last-write-wins `DELETE`. So it closes #732 **for persists** — a stale owner
cannot overwrite a newer snapshot — while leaving it **open for deletes**: during an overlap a stale
writer can still erase a newer snapshot or resurrect a just-deleted key, and with events-recovery a
crash alone can revive one (the same revive that exists under plain last-write-wins). That residual is a
documented boundary, not an oversight, and it is pinned as such — modelled here (`cassandra_notomb` =
`Guarded ∧ ¬Tombstone`, VIOLATES `INV_NoCorruptDurable`; claims X1) and integration-tested on the
persist-only branch itself (`SnapshotSpec`, "a delete is unguarded … persist-only gap"; that IT is not
on this research branch). Fencing the delete is what pulls in the offset-carrying tombstone, the
monotonic buffer, tombstone-floor recovery and the events-recovery floor (the F-3/F-6/F-7 machinery;
introducing it also surfaced the F-2 wiring regression) — a breaking API and a liveness trap on each
recovery path. That larger design is the **full** fence — the primary subject this report audits, also
an upstream draft, also verified, also deferred. Both are verified here; neither has merged. Persist-only
is the conservative first step whose cost/benefit the audit justifies.
