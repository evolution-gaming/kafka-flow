...
| M5 | Not-applied result carries the stored `offset` when Cassandra returns it; absent column or null ⇒ "row absent" | code `resolveConditional`; ext (2),(3) — exact result shape for absent row vs null cell | ⏳ ext |
...
| R6 | `SnapshotFold` filter (`record.offset > snapshot.offset`) keeps only records past the recovered offset. In the full design it makes monotonic-append belt-and-suspenders for persists (tick-delete bypasses it, so the buffer is the only protection there); in the **persist-only mode it is the *primary* liveness mechanism** — there is no monotonic buffer, so it alone keeps the owner from re-persisting below its recovered high-water | code `SnapshotFold`, `TickToState`; test `SnapshotFoldSpec` (equal-offset drop) | ✅ |
...
| T4 | TTL is set on both the insert and update paths | IT `SnapshotSpec` TTL tests; code `ttlFragment` in all five CAS-mode write statements (persist, repair, first-write INSERT, tombstone INSERT, delete) | ✅ |
...
| KF2 | Snapshot write and input-offset commit are bound in one transaction; the committed offset never leads the durable snapshot (bounded by one in-flight round) | model `kafka_replay` (`INV_CommittedNeverAhead`, `LIVE_CommitCatchesUp`), negatives `kafka_lag_nofilter`, `kafka_replay_unbound_gap`; model-fidelity K1 | ✅ |
...
| KF5 | The publish guard never emits the −1 unknown-generation sentinel (which would commit unfenced) | ext(K3) source; refresh study; `ConsumerSpec` (unit: the −1 sentinel is dropped) | ✅ |
...
| KF8 | Every flow alive after a poll is owned in the refreshed generation (the sole cross-partition net) | documented `ConsumerRebalanceListener` contract (sync revoke/lost-before-assign) + awaited teardown; [`cassandra-report.md`](cassandra-report.md) §10.1; coupling modelled in `FlowsAlive` (`INV_FlowsAlive` as safety; neg `flowsalive_race`) and pinned by unit `TopicFlowSpec` "remove awaits the flow teardown" (with the generation-refresh change); the invariant's **eviction boundary** (teardown past the rebalance timeout, where the broker is the only net) realized end to end in the real-eviction teardown experiment ([`kafka-generation-study.md`](kafka-generation-study.md)) | ✅ by contract; ✅ modelled + tested; ✅ eviction boundary realized |
...
| KF17 | The two nets are ordered: a batch carrying ≥1 write is rejected on the produce ack (the shared per-partition `transactional.id`'s bumped epoch) before `commitOffsets` runs, so on an eviction member/generation validation is **unreachable** — reached only by an offset-only marker batch, which produces nothing. Safety-neutral (KF9 stands); load-bearing for evidence, since pinning the generation net on an eviction needs a marker-only batch or it measures the epoch net instead | ext(K5) (ordering, source-pinned there) + `GroupCommit.commitBatch` order; corroborated on the eviction route by the real-eviction teardown experiment ([`kafka-generation-study.md`](kafka-generation-study.md)), which recorded one broker-answered call — `send-ack → InvalidProducerEpochException` — and no `sendOffsetsToTransaction` | ✅ composed + corroborated (experiment, not a retained pin) |
