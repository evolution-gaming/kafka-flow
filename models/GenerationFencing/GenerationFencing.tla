...
(* generation fencing (KIP-447), the load-bearing capture-coupling invariant   *)
(* (INV_CaptureCoupled), and the load-bearing offset SEED.                     *)
(*                                                                             *)
...
(*                                                                             *)
(*   Capture-coupling (INV_CaptureCoupled): whenever a flow can still flush    *)
(*   a (stale) snapshot, the generation its consumer observes is not ahead     *)
(*   of the generation its live flow recovered at.                             *)
(*                                                                             *)
...
VARIABLES gen, realOwner, alive, flowGen, capturedGen, carriesOffset, durableGen, corrupted,
  ...
  \* capturedGen[f]   : generation f's consumer last observed, from Consumer.groupMetadata; 0 = none
...
vars == <<gen, realOwner, alive, flowGen, capturedGen, carriesOffset, durableGen, corrupted,
          txnOpen, txnFlow, txnGen>>
...
Init ==
  ...
  /\ capturedGen    = [f \in Flows |-> IF f = "A" THEN 1 ELSE 0]
...
Rebalance(to) ==
  ...
  /\ gen' = gen + 1
  /\ realOwner' = to
  /\ UNCHANGED <<alive, flowGen, capturedGen, carriesOffset, durableGen, corrupted, txnOpen, txnFlow, txnGen>>
...
Poll(f) ==
  /\ capturedGen[f] /= gen
  /\ capturedGen' = [capturedGen EXCEPT ![f] = gen]
  /\ IF Coupled
       THEN \* teardown is coupled to the capture: crossing a generation boundary closes the flow's
            \* producer, which ABORTS its in-flight transaction (txnOpen -> FALSE). This is the
            \* transaction-level analogue of INV_CaptureCoupled -- a flow can never commit a batch
            \* staged in an older generation.
...
ScheduleCommit(f) ==
  ...
  /\ UNCHANGED <<gen, realOwner, alive, flowGen, capturedGen, durableGen, corrupted, txnOpen, txnFlow, txnGen>>

\* --- Single-write flush (Batched = FALSE) -------------------------------------------------
\* A live flow flushes a snapshot in a transaction. If it carries an offset, KIP-447 gates it:
\* durable iff capturedGen[f] = gen. If it carries no offset (unseeded, pre-first-commit), it is ungated.
Flush(f) ==
  /\ ~Batched
  /\ alive[f]
  /\ IF carriesOffset[f] /\ capturedGen[f] /= gen
       THEN UNCHANGED <<durableGen, corrupted>>                 \* fenced
       ELSE /\ corrupted'  = (corrupted \/ (flowGen[f] < durableGen))
            /\ durableGen' = flowGen[f]
  /\ UNCHANGED <<gen, realOwner, alive, flowGen, capturedGen, carriesOffset, txnOpen, txnFlow, txnGen>>
...
BeginStage(f) ==
  ...
  /\ txnFlow' = f
  /\ txnGen'  = flowGen[f]
  /\ UNCHANGED <<gen, realOwner, alive, flowGen, capturedGen, carriesOffset, durableGen, corrupted>>
  ...
  /\ LET f == txnFlow IN
       IF carriesOffset[f] /\ capturedGen[f] /= gen
         ...
         ELSE /\ corrupted'  = (corrupted \/ (txnGen < durableGen))
              /\ durableGen' = txnGen
  /\ txnOpen' = FALSE
  /\ UNCHANGED <<gen, realOwner, alive, flowGen, capturedGen, carriesOffset, txnFlow, txnGen>>
...
----------------------------------------------------------------------------
\* Capture-coupling: a live flow's recovered generation equals its consumer's captured generation.
INV_CaptureCoupled == \A f \in Flows : alive[f] => (flowGen[f] = capturedGen[f])
