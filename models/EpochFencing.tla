...
VARIABLES gen, realOwner, alive, flowGen, epoch, maxEpoch, durableGen, corrupted, ownerFenced
  ...
  \* flowGen[f]  : generation f's live flow recovered at (freshness); 0 = none
  ...
  \* durableGen  : freshness of the snapshot currently durable
  \* corrupted   : a staler-freshness write became durable over a fresher one (#732)
...
\* The current owner recovers a live flow at the current generation (its freshness). A stale owner is
...
\* #732 safety: no stale (older-freshness) snapshot ever overwrites a newer durable one.
