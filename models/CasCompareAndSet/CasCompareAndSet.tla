...
(* PR #828, focused on the offset-gated delete (the new CAS mode).             *)
(*                                                                             *)
(* Abstraction:                                                                *)
(*  - One key. Its stored cell is a linearizable register (Paxos LWT), so      *)
(*    every persist/delete is an atomic compare-and-set on it.                 *)
(*  - Deterministic folds (a user contract): the operation at log offset o     *)
...
(*                 ~GuardedDelete (unconditional): applies if present          *)
(*                                                                             *)
(* ASSUMES: the cell is a per-key linearizable register (Paxos LWT), so each   *)
(* persist/delete is atomic; folds are DETERMINISTIC, so op[o] is a pure       *)
    ...
    ELSE \* delete
      IF ~stored.present
        ...
        ELSE IF GuardedDelete
               ...
               ELSE Absent                            \* unconditional: DELETE IF EXISTS
...
\* keys on the writer named "owner"; the configs' Writers set must contain it
