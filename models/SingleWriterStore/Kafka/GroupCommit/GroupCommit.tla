...
(* writer to take the per-partition lock drains what is queued (up to a    *)
(* cap) into one transaction and delivers each waiter's outcome.           *)
(*                                                                         *)
(* One spec, two properties on the same machine:                           *)
(*   LIVENESS Termination : every queued write's outcome is delivered --   *)
(*       no stranded writer, no deadlock -- under weak fairness on the     *)
(*       writers and the flow.  This is the suite's clearest progress      *)
(*       property.                                                         *)
(*   SAFETY INV_OffsetWithinDurable : the committed input offset never     *)
(*       leads the durable write prefix, even when a flush splits across   *)
(*       capped batches (flush-blocks-then-schedule).                      *)
(*                                                                         *)
...
(* --algorithm GroupCommit {
     variables queue = <<>>,   \* FIFO of offered-but-not-yet-committed write
                               \* ids
               lock = FALSE,   \* the single transaction lock (Semaphore(1))
               done = {},      \* committed (durable) write ids
               oc = 0,         \* offsetToCommit: the scheduled offset
                               \* (raised by the flow)
     ...
     define {
       ...
       \* writes for offsets 1..o all durable
       DurablePrefix(o) == \A k \in 1 .. o : k \in done
     }

     \* a snapshot write: offer to the queue, take the lock, commit a batch
     \* (completing each item's outcome and binding the latest offset), then
     \* await its own outcome.
     fair process (w \in Writers) {
       offer:   queue := Append(queue, self);
       acq:     await ~lock; lock := TRUE;
       commit:  done  := done \cup AsSet(Batch);
                \* every transaction binds the latest offset
                co    := oc;
                queue := Rest;
                lock  := FALSE;
     ...
     \* the flow schedules the input offset to commit -- only after the
     \* writes it covers are durable (flush-blocks-then-schedule). Gated =
     \* FALSE removes that coupling (the hazard).
     fair process (flow = 0) {
       sched:   while (oc < N) {
                  with (o \in (oc + 1) .. N) {
