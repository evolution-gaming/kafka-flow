...
(* a behaviour the atomic spec allows.  The only deviation from atomic is  *)
(* a *spurious* conflict -- the retry finding the row gone (a TTL reap /   *)
(* LWW hard delete between INSERT and retry) -- which is liveness-only     *)
(* (the flow recovers on its next flush) and maps to the atomic spec's     *)
(* GiveUp.                                                                 *)
(*                                                                         *)
(*   Guarded = FALSE : ungated UPDATEs -> INV_NoStaleOverwrite fails (the  *)
(*             offset guard is load-bearing even in the compound).         *)
(*   Reap    = TRUE  : a present row may be removed, making the            *)
(*             spurious-conflict path reachable (INV_NeverSpurious fails   *)
(*             -- a reachable but liveness-only deviation, safety still    *)
(*             holds).                                                     *)
(*   SpecGuarded : the ABSTRACT spec's guard, normally = Guarded.          *)
(*             Decoupled only so the refinement check itself can be shown  *)
(*             non-vacuous: an ungated compound against the guarded atomic *)
(*             spec must FAIL RefinesAtomic (casfw_refines_vacuous) -- a   *)
(*             mapping that let that pass would be accepting anything.     *)
(***************************************************************************)
EXTENDS Naturals
...
VARIABLES stored, pc, off, corrupted, spurious
  \* stored      : the key's cell, a linearizable register (each LWT is
  \*               atomic)
  \* pc[w]       : "u1" -> "done"/"conflict" (row present); else "u1" ->
  \*               "ins" -> "done" (INSERT won), or -> "u2" ->
  \*               "done"/"conflict" (a row appeared underneath)
  \* off[w]      : the offset w is persisting (fixed per writer)
  \* corrupted   : a stale overwrite happened (a present, strictly-higher
  \*               cell changed)
  \* spurious[w] : w hit the spurious-conflict path (retry found the row
  \*               gone)
...
----------------------------------------------------------------------------
\* Safety: the non-atomic compound never overwrites a strictly-higher present
\* cell.
INV_NoStaleOverwrite == ~corrupted

\* Refinement (Sec. 5.8): the compound implements the single-atomic-CAS spec.
\* Mapping: a writer is "done" in the atomic spec once the compound has
\* finished -- written ("done") or given up ("conflict"); every other pc
\* value ("u1","ins","u2") maps to "todo", so a step between two of them
\* stutters.  stored / off / corrupted map
\* through unchanged.  Under this mapping every compound step is an atomic
\* Persist, an atomic GiveUp, a ReapRow, or a stuttering step -- so
\* CasFirstWrite => CasFirstWriteAtomic.
AbstractPc ==
  [w \in Writers |->
     IF pc[w] \in {"done", "conflict"} THEN "done" ELSE "todo"]
Atomic == INSTANCE CasFirstWriteAtomic
            WITH pc <- AbstractPc, Guarded <- SpecGuarded
RefinesAtomic == Atomic!Spec

\* Used only to show the spurious-conflict path is REACHABLE (expected
\* violated under Reap): its counterexample IS the spurious conflict; safety
\* still holds.
