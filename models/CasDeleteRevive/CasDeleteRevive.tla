...
(*                                                                             *)
(* Tombstone = FALSE : the row-removing delete (DELETE removes the row).       *)
(* Tombstone = TRUE  : the offset-carrying tombstone (shipped): delete writes  *)
...
OwnerFold ==
  ...
  /\ ownerPos' = ownerPos + 1
  /\ corrupted' = (corrupted \/ Corrupt(stored'))   \* stored' is the value assigned in the LET above
  /\ UNCHANGED op
...
\* A lagging zombie flushes its correct-as-of-m snapshot late (CAS-gated). Internally consistent
