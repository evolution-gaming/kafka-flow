...
class FlowSpec extends CassandraSpec {
  ...
  test(
    ...
    val events  = (1 to 6).toList.map(i => s"e$i") // offsets 0..5 -> recovered snapshot offset 5
    ...
    val test: IO[(Either[Throwable, Unit], Option[KafkaSnapshot[String]])] = for {
      ...
      _                 <- releaseA
      stored            <- storage.snapshots.get(KafkaKey(appId, groupId, tp, key))
      _                  = assertEquals(clue(stored.map(_.offset)), Some(Offset.unsafe(5)))
      // a new flow recovers the key (snapshot offset 5) at assignedAt = 0; a replayed "DELETE" at offset 2 (< 5)
      // makes the fold return None -> delete at current.offset 2, which must be fenced on the high-water 5
      flowB             <- allocateStaleFlow(storage, appId, groupId, tp, deleteOnMarkerFold)
      (flowB_, releaseB) = flowB
      deleteResult <- flowB_(staleFlowRecords(List("DELETE"), key, tp).map(_.copy(offset = Offset.unsafe(2)))).attempt
      _            <- releaseB.attempt
